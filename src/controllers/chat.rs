...
pub async fn msg<'r>(user: Jwt, req: Json<MsgRequest>, chat_server: &State<EventServer>) -> NetworkResponse<'r, String> {
    ...
    let destination_tx = match chat_server.get_session_tx(&message_req.destination).await {
        None => {
            ...
            chat_server.get_session_tx(&message_req.destination).await.unwrap()
        }
        Some(tx) => tx
    };
    ...
    match destination_tx.send(event) {
        Ok(_) => {
            NetworkResponse::Ok(Json(Response::<String> {
                error_code: None,
        ...
        Err(_) => {
            NetworkResponse::InternalServerError(Json(Response::<String> {
                error_code: Some(500),
